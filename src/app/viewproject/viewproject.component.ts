...
export class ViewprojectComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.addprojectService.getDataforDivcircity().subscribe(data => {
      ...
      const mid = this.project.length > 0 ? this.project[0].mid : null;

      if (mid) {
        this.addprojectService.getdataforeditmid(mid).subscribe(devWorkDetails => {
          console.log(devWorkDetails);
          this.projects = devWorkDetails.developmentWorks;
        });
      }
    ...
    this.uploadService.getFiles().subscribe(
      (data: any) => {
        ...
        console.log(pronamecomment);
        this.fileInfos = data.filter(s => s.name === pronamecomment && (s.type === 'image/jpeg' || s.type === 'image/png'));
