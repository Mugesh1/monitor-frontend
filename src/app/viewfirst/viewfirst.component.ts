...
@Component({
  selector: 'app-viewfirst',
  templateUrl: './viewfirst.component.html',
  styleUrls: ['./viewfirst.component.css']
})
export class ViewfirstComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.addprojectService.getDataforDivcircity().subscribe(data => {
      ...
      this.projectone = data.filter(s => s.project_name === this.route.snapshot.paramMap.get('project'));
      ...
      let index = _.unionBy(this.projectall, 'dueDateOfCompletion');
      this.projectonefordue = index;
      const duedate = this.projectonefordue.map(s => s.dueDateOfCompletion)
  ...
  Delete(project_name: string): void {

    if (window.confirm('Are you sure  to delete this project ?')) {
      this.addprojectService.delete(project_name)
        .subscribe(
          response => {
  ...
  updateProjectComment(): void {
    ...
    this.message = '';
    this.addprojectService.updateProject(mid, data)
      .subscribe(
